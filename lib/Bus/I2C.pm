...
sub enter_i2c {  $_[0]->send({message=>"\x02", delay=>.1}) eq "I2C1" : return 1 : return 0; }
...
sub i2c_command {
  ...
  my $data = $self->i2c_bulk_transfer(2, [i2caddr<<1, cmd]);
...
sub i2c_get_byte {
  ...
  $self->i2c_send_start_bit();
  my $data = $self->i2c_bulk_transfer(2, [i2caddr<<1, addr]);
  $self->i2c_send_start_bit();
  $data += $self->i2c_bulk_transfer(1, [i2caddr<<1 | 1]);
...
sub  i2c_set_word {
  ...
  my $vl = $val%256;
  $self->i2c_send_start_bit();
  my $word = $self->i2c_bulk_transfer(4, [i2caddr<<1, addr, vh, vl]);
  $self->i2c_send_stop_bit();
  return $word;
}
...
around 'set_word' => sub {
  ...
  $self->throw({exception=>'IOError', messsage=>"I2C command $cmd on address $i2c_addr not acknowledged!"}) unless $status =~ /x01/ == -1;
  $self->log_debug("Data for cmd: $cmd  => $data");
  return $word;
}
...
around 'i2c_command' => sub {
  ...
  $self->throw({exception=>'IOError', messsage=>"I2C command $cmd on address $i2c_addr not acknowledged!"}) if $data-> [0] == 0x01;
  $self->log_debug("Data for cmd: $cmd  => $data");
  return $results;
}
